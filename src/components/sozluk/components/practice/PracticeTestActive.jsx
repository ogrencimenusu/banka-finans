        ...
        <>
            <Container fluid className="py-4 bg-transparent">
                ...
                <div className="d-md-none mb-3">
                    <button 
                        className="btn btn-primary d-flex align-items-center gap-2 rounded-pill shadow-sm py-2 px-3 fw-bold"
                        onClick={() => setShowMobileMenu(true)}
                        title="Soruları Göster"
                        style={{ fontSize: '14px' }}
                    >
                        <i className="bi bi-list-check fs-5"></i>
                        <span>Test Soruları</span>
                    </button>
                </div>
                ...
                <Row className="position-relative">
                    ...
                    <Col md={3} lg={2} className="p-0 mb-4 mb-md-0 d-none d-md-block" style={{ position: 'sticky', top: '100px', height: 'fit-content', zIndex: 100 }}>
                        {hasWrittenQuestions && (
                            <Card className="bg-light border-secondary border-opacity-25 rounded-4 p-3 shadow-none text-body mb-3">
                        ...
                        <Card className="bg-light border-secondary border-opacity-25 rounded-4 p-3 shadow-none text-body d-flex flex-column" style={{ maxHeight: 'calc(100vh - 300px)' }}>
                                    ...
                                    <div className="d-flex flex-column gap-2 pb-4">
                                        {questions.map((q, idx) => {
                                            ...
                                            return (
                                                <div
                                                    key={`sidebar-ans-${idx}`}
                                                    className={`p-2 rounded-3 border ${isAnswered ? (completed ? (answers[idx]?.selected?.isCorrect ? 'border-success bg-success bg-opacity-10' : 'border-danger bg-danger bg-opacity-10') : 'border-primary bg-primary bg-opacity-10') : 'border-secondary bg-light'} border-opacity-25 transition-all cursor-pointer`}
                                                    onClick={() => { scrollToQuestion(idx); }}
                    ...
                    <Col md={9} lg={8} className="pb-5 mobile-questions-container" style={{ marginLeft: 'clamp(20px, 4vw, 60px)' }}>
                        ...
                        {completed && (
                            <Card className="bg-white border-0 rounded-4 shadow-sm mb-5 text-start overflow-hidden border">
                                <div className="p-4 p-md-5">
                                    ...
                                    <Row className="g-5 align-items-center">
                                        ...
                                        <Col lg={6} className="d-flex flex-column gap-4">
                                            <div className="d-flex flex-column align-items-center gap-4 bg-light bg-opacity-50 p-4 rounded-4 border border-secondary border-opacity-10">
                                                {/* Circular Progress */}
                                                <div className="position-relative flex-shrink-0 mx-auto" style={{ width: '160px', height: '160px' }}>
                                                    <svg width="160" height="160" viewBox="0 0 160 160">
                                                        <circle cx="80" cy="80" r="70" fill="transparent" stroke="#e9ecef" strokeWidth="12" />
                                                        <circle cx="80" cy="80" r="70" fill="transparent" stroke={scorePercent >= 70 ? "#20c997" : (scorePercent >= 40 ? "#ffc107" : "#dc3545")} strokeWidth="12" strokeDasharray={2 * Math.PI * 70} strokeDashoffset={2 * Math.PI * 70 - (scorePercent / 100) * (2 * Math.PI * 70)} strokeLinecap="round" style={{ transform: 'rotate(-90deg)', transformOrigin: '50% 50%', transition: 'stroke-dashoffset 1.5s ease-out' }} />
                                                    </svg>
                                                    <div className="position-absolute top-50 start-50 translate-middle text-center w-100">
                                                        <div className="fw-bold display-5 text-dark lh-1 mb-1">%{scorePercent}</div>
                                                        <div className="text-muted fw-bold" style={{ fontSize: '12px', letterSpacing: '1px' }}>BAŞARI</div>
                                                    </div>
                                                </div>

                                                {/* Metrics list */}
                                                <div className="d-flex flex-column gap-2 w-100 justify-content-center">
                                                    <div className="d-flex align-items-center justify-content-between bg-white px-3 py-2 rounded-3 border-start border-success border-4 shadow-sm">
                                                        <span className="fw-bold text-secondary" style={{ fontSize: '14px' }}>Doğru</span>
                                                        <span className="fw-bold text-success fs-5 lh-1">{correctCount}</span>
                                                    </div>
                                                    <div className="d-flex align-items-center justify-content-between bg-white px-3 py-2 rounded-3 border-start border-danger border-4 shadow-sm">
                                                        <span className="fw-bold text-secondary" style={{ fontSize: '14px' }}>Yanlış</span>
                                                        <span className="fw-bold text-danger fs-5 lh-1">{incorrectCount}</span>
                                                    </div>
                                                    <div className="d-flex align-items-center justify-content-between bg-white px-3 py-2 rounded-3 border-start border-secondary border-4 shadow-sm">
                                                        <span className="fw-bold text-secondary" style={{ fontSize: '14px' }}>Boş</span>
                                                        <span className="fw-bold text-secondary fs-5 lh-1">{skippedCount}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {(initialTestState?.config?.advancedOptions?.comboStreak || initialTestState?.config?.advancedOptions?.timeSurvival || initialTestState?.config?.advancedOptions?.progressiveHint) && (
                                                <div className="d-flex gap-3 flex-wrap">
                                                    {initialTestState?.config?.advancedOptions?.comboStreak && (
                                                        <div className="d-flex align-items-center gap-2 bg-warning bg-opacity-10 px-3 py-2 rounded-pill border border-warning border-opacity-25">
                                                            <i className="bi bi-fire text-warning"></i>
                                                            <span className="small fw-bold text-warning-emphasis">{maxStreak}x Combo</span>
                                                        </div>
                                                    )}
                                                    {initialTestState?.config?.advancedOptions?.timeSurvival && (
                                                        <div className="d-flex align-items-center gap-2 bg-success bg-opacity-10 px-3 py-2 rounded-pill border border-success border-opacity-25">
                                                            <i className="bi bi-stopwatch-fill text-success"></i>
                                                            <span className="small fw-bold text-success">{timeLeft}s Kaldı</span>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                        ...
                                        <Col lg={6} className="border-start-lg border-secondary border-opacity-10 ps-lg-4">
                                            <h6 className="fw-bold text-muted text-uppercase mb-3" style={{ fontSize: '12px', letterSpacing: '0.5px' }}>Hızlı Tekrarla</h6>
                                            <div className="d-flex flex-column gap-3">
                                                <Button variant="white" className="d-flex align-items-center rounded-4 p-3 text-start border shadow-sm transition-all hover-shadow" onClick={() => {
                                                    ...
                                                    <div className="flex-grow-1">
                                                        <div className="fw-bold text-dark mb-1">Aynı Testi Yeniden Çöz</div>
                                                        <div className="text-muted small">Aynı kelimelerle testi tekrarla.</div>
                                                    </div>
                                                    <i className="bi bi-chevron-right text-muted opacity-50"></i>
                                                </Button>

                                                {missedQuestions.length > 0 && (
                                                    <Button variant="white" className="d-flex align-items-center rounded-4 p-3 text-start border border-danger border-opacity-25 bg-danger bg-opacity-10 shadow-sm transition-all hover-shadow" onClick={() => onRetakeMissed(missedQuestions)}>
                                                        <div className="bg-danger bg-opacity-25 text-danger rounded-circle d-flex align-items-center justify-content-center me-3" style={{ width: '48px', height: '48px' }}>
                                                            <i className="bi bi-exclamation-triangle fs-4"></i>
                                                        </div>
                                                        <div className="flex-grow-1">
                                                            <div className="fw-bold text-danger mb-1">Sadece Hataları Çöz</div>
                                                            <div className="text-danger text-opacity-75 small">{missedQuestions.length} soruyu tekrarla.</div>
                                                        </div>
                                                        <i className="bi bi-chevron-right text-danger opacity-50"></i>
                                                    </Button>
                                                )}
                                                ...
                                                <Button variant="white" className="d-flex align-items-center rounded-4 p-3 text-start border shadow-sm transition-all hover-shadow" onClick={() => {
                                                    ...
                                                    <div className="flex-grow-1">
                                                        <div className="fw-bold text-dark mb-1">Yeni Test Çöz</div>
                                                        <div className="text-muted small">Farklı kelimelerle yeni test başlat.</div>
                                                    </div>
                                                    <i className="bi bi-chevron-right text-muted opacity-50"></i>
                                                </Button>
                                            </div>
                                ...
                                <Row className="g-4">
                                    ...
                                    <Col lg={12}>
                                        <div className="p-3 rounded-4 bg-light border border-secondary border-opacity-10 shadow-sm">
                                                    ...
                                                    return (
                                                        <div key={cat.key} className={`border rounded-4 overflow-hidden transition-all duration-300 ${cat.borderColor} ${isExpanded ? 'shadow-sm' : ''}`}>
                                                            ...
                                                            <Collapse in={isExpanded}>
                                                                <div>
                                                                    <div className="p-3 bg-light border-top border-secondary border-opacity-10">
                                                                        {cat.count === 0 ? (
                        ...
                        {completed && (
                            <Card className="bg-light border-secondary border-opacity-25 rounded-4 p-4 shadow-sm mb-5 text-start">
            ...
            <Offcanvas 
                ...
                placement="start"
                className="bg-light border-end border-opacity-10"
                style={{ width: '280px' }}
            >
