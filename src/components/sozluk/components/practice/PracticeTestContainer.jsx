...
const PracticeTestContainer = forwardRef((props, ref) => {
    const { 
        ...
        onUpdateStage, onUpdateStagesBatch, onUpdateStatusBatch, onToggleStar, onToggleStarBatch, onDelete, onEdit, 
    ...
    const handleStart = async (rawConfig, forcedWordsArg = null) => {
        const forcedWords = forcedWordsArg || rawConfig.forcedWords || null;
        const config = {
            ...rawConfig,
            testHelps: savedOptions?.testHelps || rawConfig.testHelps || { showLetterCounter: true, colorOnLengthMatch: true, colorOnExactMatch: true }
        };
        ...
        // Firestore does not accept undefined values. Strip them out completely.
        const testData = JSON.parse(JSON.stringify(rawTestData));
            ...
            {testState === 'running' && (
                initialTestState?.config?.advancedOptions?.matchPairs ? (
                    ...
                    <PracticeTestActive
                        ...
                        onToggleStarBatch={onToggleStarBatch}
                        onDelete={onDelete}
                        onEdit={onEdit}
